...
class Ros(object):
    ...
    def close(self):
        """Disconnect from the roscore node."""
    ...
    def send_on_ready(self, message):
        """Send message to the roscore node once the connection is established.
    ...
    def call_sync_service(self, message, timeout):
        """Send a blocking service request to the roscore node once the connection is established,
    ...
    def call_async_service(self, message, callback, errback):
        """Send a service request to the roscore node once the connection is established.
